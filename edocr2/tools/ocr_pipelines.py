...
class Pipeline:
    ...
    def recognize_dimensions(self, box_groups, img):
        ...
        for box in box_groups:
            ...
            if len(cnts)==1:
                img_croped = deskew_crop(img_croped, cv2.ROTATE_90_COUNTERCLOCKWISE)
...
def deskew_crop(img, portrait_rotation=cv2.ROTATE_90_COUNTERCLOCKWISE):
    """Coarse + fine deskew of a text crop before CRNN recognition.

    Coarse step: if height > 1.15 * width (portrait), rotate 90° to make it landscape.
    Fine step: use minAreaRect on text pixels to detect and correct residual tilt (0.5°–15°).
    """
    ...
    # Coarse: portrait → landscape
    if h > w * 1.15:
        img = cv2.rotate(img, portrait_rotation)
        h, w = img.shape[:2]

    # Fine: detect residual angle via minAreaRect on ink pixels
    ...
    angle = rect[2]          # cv2.minAreaRect returns angle in [-90, 0)
    rw, rh = rect[1]
    if rw < rh:              # long axis is vertical → remap to rotation around horizontal
        angle = 90 + angle

    # Only correct small tilts; ignore near-zero or large angles
...
def ocr_dimensions(img, detector, recognizer, alphabet_dim, frame, dim_boxes = [], cluster_thres = 20, language = 'eng', max_img_size = 2048, backg_save = False):
    ...
    for d in dim_boxes:
        ...
        if x + d.w < frame.x + frame.w and y + d.h < frame.y + frame.h:
            ...
            roi = deskew_crop(roi, cv2.ROTATE_90_CLOCKWISE)
            p = recognizer.recognize(image = roi)
